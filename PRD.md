...
| **v2.21 (본 개정)** | **LV-5·LV-6을 타임라인 배정 생성 경로에도 동일 적용 + 서버단 강제로 강화(재발 방지)**, **LV-8 신설: 차감 원천을 저장값이 아닌 조회 시점 실시간 FIFO 재계산 값으로 변경(나중에 생긴 적립이 과거 지정휴가 선사용분의 원천으로 자동 재배정)**, **T-16 정식 항목 신설: 드릴다운 조건부 스크롤 로직을 폐기하고 타임라인 진입 시 진입 경로 무관하게 항상 오늘 위치로 스크롤하도록 단순화(반복 재발 버그 수정)** |
