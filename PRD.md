...
핵심 원칙: 팀 제도로 정당 적립된 휴가(프로젝트휴가 등)와 법정연차는 **합산되는 별개 권리가 아니라, 둘 중 더 큰 쪽이 총 휴가 권리**가 된다. 단, **주말/휴일대체 휴가는 실제 근무에 대한 법정 성격의 보상이므로, 법정연차 쪽 후보에도 함께 더해서 비교**한다. 즉 구성원은 max(법정연차 누적 + 주말/휴일대체 누적, 팀 정당 적립 누적) 이상의 휴가 권리를 가지며, 그 범위 내 사용은 퇴사 시 불이익이 없다.

| ID | 요구사항 |
|---|---|
| AL-3 | **총 휴가 권리(Entitlement)** = **max( 법정연차 누적 합(AL-2, 보정 포함) + 주말/휴일대체 누적 적립 합, 팀 정당 적립 합(프로젝트휴가·주말/휴일대체·포상·특별·지연보상 등 Leave 탭의 적립분 전체) )**. **주말/휴일대체는 팀 정당 적립 합에도 이미 포함되어 있으나, 법정연차 쪽 후보 계산에도 별도로 더해서 두 후보값을 비교**한다(어느 한쪽 합계에 두 번 누적되는 것이 아니라, max 비교를 위한 두 후보 각각에 포함되는 것). 법정연차+주말/휴일대체 합이 팀 정당 적립 합보다 크면 그 값을 총 휴가 권리로 한다. |
| AL-4 | **초과 사용분(퇴사 시 차감)** = max(0, 총 유급 사용일수 − 총 휴가 권리(AL-3)). 정당 권리 범위 내 사용은 차감하지 않는다. |
| AL-5 | **미달 보상분(퇴사 시 보상)** = max(0, 총 휴가 권리(AL-3) − 총 유급 사용일수). 권리보다 적게 썼으면 그 차이만큼 보상한다. |
| AL-6 | **퇴사 정산 뷰**(상세 내역 필수 표시, 합계 숫자만 아님): 인력·기준일(퇴사일) 선택 시 다음을 표로 표시한다. ① **법정연차 적립·보정 "내역"**(연도별 적립일수 + 보정 항목 각각과 사유를 **펼친 리스트**로), ② **팀 정당 적립 "내역"**(유형별 적립 일수와 발생 원천을 **펼친 리스트**로 — 주말/휴일대체 항목이 포함됨), ③ **총 휴가 권리 비교**: **(a) 법정연차+주말/휴일대체 = X**, **(b) 팀 정당 적립 합 = Y**를 나란히 보여주고 **총 휴가 권리 = max(X, Y)**, 어느 쪽(a/b)이 기준이 되었는지 표시, ④ **총 유급 사용 "내역"**(사용 일자·기간·휴가 유형·차감 원천을 포함한 **펼친 리스트**로), ⑤ 초과 사용분(AL-4), ⑥ 미달 보상분(AL-5), ⑦ 최종 정산(⑥−⑤). **⑤~⑦의 합계 수치는 ①②④ 리스트의 합계와 정확히 일치해야 하며 별도 재계산하지 않는다.** 퇴사자(termination_date 보유) 조회 가능. |
...
| **v2.18 (본 개정)** | **AL-3 산식 수정: 총 휴가 권리 = max(법정연차 누적, 팀 정당 적립 누적)에서 max(법정연차 누적 + 주말/휴일대체 누적, 팀 정당 적립 누적)으로 변경. 법정연차+주말/휴일대체 합이 팀 정당 적립 합보다 크면 그 값을 기준으로 함(AL-6 퇴사 정산 뷰도 두 후보값을 나란히 표시하도록 개편)** |
