...
| **v2.7 (본 개정)** | **타임라인 렌더 윈도우 제한**(지정 시 기간 ±1개월, 미지정 시 당일 ±7개월만 표시, T-10), **로고를 assets/logo.png(정사각 아이콘)로 사용**(§9.0), **대시보드 프로젝트 Kick-off·종료 목록 추가**(지난주·이번주·다음주), **보안 점검·강화 섹션 추가(§12)** |
