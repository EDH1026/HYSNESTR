...
| **T-10** | **렌더 윈도우 제한**: 기간 필터 활성 시 뷰 범위를 필터 ±1개월로 제한, 필터 없을 때는 오늘 ±7개월. 무한 스크롤 대신 고정 캔버스 폭 사용. |
| **T-11** | **기간 프리셋 버튼**: 기간 필터 옆 원클릭 버튼 — 이번 주(월~일), 이번 달(1일~말일), 이번 분기(FY 기준 Q, 7월 시작), 이번 FY(해당 FY 7/1~6/30). 클릭 시 필터·렌더 윈도우 즉시 적용, 활성 상태 시각 표시, 재클릭 시 해제. |
| **T-12** | **배정 막대 우클릭 컨텍스트 메뉴**: 편집·복제·삭제·작업 상세 열기·이 사람 휴가 보기. 복제=직후 기간에 새 배정 생성(AssignmentModal 사전입력). 편집성 항목은 editor/admin만 표시, Closed 작업 배정에는 비활성(Closed 잠금). 삭제=확인 후 실행+Undo 가능. viewer에게는 조회성 항목(작업 상세, 본인 휴가)만 노출. 바깥 클릭·Esc로 닫힘. |
| **T-13** | **드래그 중 날짜 미리보기 툴팁**: 막대 이동·리사이즈 중 커서 근처에 "MM/DD → MM/DD · N일"(work=달력일, 휴가=영업일 N일) 실시간 표시. 드래그 종료 시 제거. |
| **T-14** | **다중 선택 일괄 이동**: Shift/Ctrl+클릭으로 배정 막대 다중 선택(선택 표시: 파란 테두리 링). 빈 곳 클릭·Esc로 선택 해제. 선택 상태에서 그중 하나를 드래그하면 선택된 모든 배정이 동일 일수만큼 함께 이동. 휴가는 영업일 스냅 규칙 유지. Closed·권한 없는 항목은 건너뜀. 전체 이동은 Undo 1단계로 취소 가능. |
...
| B-3 | 업로드 시 **미리보기 + 검증**(필수값·형식(E-00000000/LPN 5자리)·날짜·중복 LPN 등) 후 확정 반영. 오류 행은 사유와 함께 표시하고 정상 행만 선택 반영 가능. |
...
| **v2.8 (본 개정)** | **기간 프리셋 버튼(T-11)**, **배정 우클릭 컨텍스트 메뉴(T-12)**, **드래그 중 날짜 미리보기(T-13)**, **다중 선택 일괄 이동(T-14)**, **대시보드 드릴다운(D-6)**, **전역 검색 Ctrl+K(§5.11a)**, **인력·작업항목 CSV 일괄 업로드(§5.11b, admin)** |
