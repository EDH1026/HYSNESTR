...
| **LV-9** | **수동 적립의 비고 → 원천 활용**: 자동 생성 적립(예: 프로젝트휴가 — work_item에서 자동 산정)이 아니라 **특별휴가·지연보상·포상휴가 등 수동으로 추가하는 적립**은 원천(source)이 work_item으로 자동 연결되지 않으므로, **입력 시 비고(note) 작성을 필수로 하고 그 비고 내용을 원천(source) 값으로도 사용**한다. 적립 이력 표의 "원천" 열에는 수동 적립의 경우 이 비고 내용이 표시되며(예: "예비군 훈련", "조부모상"), 자동 생성 적립처럼 "—"로 공백 표시되지 않는다. |
