...
| **v2.9** | Workitem 탭 인력 더블클릭 → 인력 칩 연동(T-15) ✅, 드릴다운 진입 시 오늘 날짜로 스크롤(T-16), 소속 인력 기간에 따른 workitem 시작/종료일 자동 확장(E-5), 다중 선택 일괄 리사이즈(T-14 확장), CV 수행기간 분할 투입 정확 반영(V-4), 프로젝트휴가 적립 인력×프로젝트 투입기간 합산 기준(§7.1) |
