...
| **T-11** | **Timeline-Person 배정칩 표시(버그 수정)**: 인력별(Person) 뷰에서 각 인력 행에 해당 인력이 배정된 **작업 배정칩(assignment chip)** 이 타임라인 막대로 **반드시 렌더링**되어야 한다(현재 표시되지 않는 버그 수정). 작업별(Work Item) 뷰와 동일하게 `assignments`를 `person_id` 기준으로 그룹핑해 각 인력 행에 칩을 그린다. 칩에는 작업명·기간이 표시되며 색상은 §4의 유형 파생 색상을 따른다. 데이터 소스·좌표 계산(viewportStart 원점 + pxPerDay)은 Work Item 뷰와 공통 로직을 사용해 누락 없이 표시한다. |
...
| **v2.10 (본 개정)** | ① **Timeline-Person 배정칩 미표시 버그 수정**: 인력 뷰에 배정칩이 렌더링되지 않던 문제, Work Item 뷰와 공통 로직으로 강제 표시(T-11). ② **Closed→Open 전환을 별도 버튼이 아닌 기존 Open/Closed toggle로 수행**하도록 명확화(W-4·W-5·§6.4). ③ **대시보드 입사 예정자 추가**: hire_date가 금일 이후인 인력을 '금주 복귀 예정자'와 유사 형식으로 표기(§5.11). |
